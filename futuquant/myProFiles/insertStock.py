...
RET_OK, basic_info_table = quote_ctx.get_stock_basicinfo(market='SH', stock_type='STOCK')
...
# 第一步写入股票信息到数据库
# db = MySQLDatabase('lim', user='lim', passwd='hackch', host='lim.app')
...
for index, row in basic_info_table.iterrows():
    stockBook = StockBasicinfo(code=row["code"], name=row["name"], lot_size=row["lot_size"],
                               ...
                               owner_stock_code=row["owner_stock_code"], listing_date=row["listing_date"],
                               stockid=row["stockid"])
    stockBook.save()
